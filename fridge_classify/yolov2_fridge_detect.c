...
#define MODEL_FILE_HAND    "./data/nnie_model/detection/zt_inst.wk" //"/userdata/models/fridge_classify/fridge_detect.wk" ztong // darknet framework wk model
#define PIRIOD_NUM_MAX     49 // Logs are printed when the number of targets is detected
#define DETECT_OBJ_MAX     32 // detect max obj

static uintptr_t g_fridgeModel = 0;
...
{
    return Yolo2FdLoad(&g_fridgeModel);
}
...
{
    return Yolo2FdUnload(g_fridgeModel);
}

static HI_S32 FridgeDetect(uintptr_t model, IVE_IMAGE_S *srcYuv, DetectObjInfo boxs[])
{
...
//     const IVE_IMAGE_S *img, DetectObjInfo resBuf[], int resSize, int* resLen)
    ...
    if (ret < 0) {
        SAMPLE_PRT("Fridge detect Yolo2CalImg FAIL, for cal FAIL, ret:%d\n", ret);
        return ret;
    }
...
{
    int ret = FridgeDetect(g_fridgeModel, srcYuv, resArr);
    return ret;
}
