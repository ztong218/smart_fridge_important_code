...
static char *MakeProfileReport(WeChatProfile *payload)
{
    ...
    cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActionDetect,
                            payload->reportAction.detectActionStatus);
    /* food */
    cJSON *array = cJSON_CreateArray();
    int size_of_foodActionData = sizeof(payload->reportAction.foodActionData) / sizeof(payload->reportAction.foodActionData[0]);
    for (int i = 0; i < size_of_foodActionData; i++) {
        cJSON *item = cJSON_CreateObject();
        // 结构体的4个字段
        cJSON_AddStringToObject(item, "name", payload->reportAction.foodActionData[i].name);
        cJSON_AddNumberToObject(item, "quantity", payload->reportAction.foodActionData[i].quantity);
        cJSON_AddNumberToObject(item, "period", payload->reportAction.foodActionData[i].period);
        cJSON_AddStringToObject(item, "storageDate", payload->reportAction.foodActionData[i].storageDate);
        cJSON_AddItemToArray(array, item);  // 将 item 添加到 array 中
    }
    cJSON_AddItemToObject(reported, payload->reportAction.subDeviceActionFood, array);
...
#define CN_PROFILE_TOPICFMT_TOPIC            "$shadow/operation/DI5K7YYGE7/hi3516_mqtt"
