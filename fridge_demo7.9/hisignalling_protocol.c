...
HisignallingErrorType HisignallingMsgReceive(hi_u8 *buf, hi_u32 len)
{
    ...
    if ((buf[0] == HISIGNALLING_MSG_FRAME_HEADER_1) && (buf[1] == HISIGNALLING_MSG_FRAME_HEADER_2)) {
        category[count_rec] = buf[3];
        number[count_rec] = buf[5];
        printf("class:%x\n",category[count_rec]);
        printf("num:%x\n",number[count_rec]);
        count_rec++;
...
hi_u32 HisignallingMsgSend(char *buf, hi_u32 dataLen)
{
    ...
    void mysend()
    {
        if (sendFlag) {
                /* 准备发送数据的代码 */
                char buf[]={"a"};
                /* 调用 HisignallingMsgSend 函数发送数据 */
                printf("wait key for senduart\n");
                // getchar();
                TaskMsleep(2000);
                hi_u32 result = HisignallingMsgSend(buf, 1);
